...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    const { orderId, customerName, customerPhone, totalAmount, trackingUrl }: OrderSMSRequest = await req.json();
    ...
    // Sanitize customer name for SMS
    const safeName = customerName.substring(0, 50).replace(/[<>]/g, '');
    const message = `Hi ${safeName}! Your order #${orderId.slice(0, 8)} for $${totalAmount.toFixed(2)} has been confirmed. Track your order: ${trackingUrl}`;
